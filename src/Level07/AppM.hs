...
instance Applicative (AppM e) where
  pure :: a -> AppM e a
  pure x = AppM $ \_ -> return $ return $ x

  (<*>) :: AppM e (a -> b) -> AppM e a -> AppM e b
  (<*>) (AppM f) (AppM m) = AppM $ \e -> do
    mRes <- m e
    fRes <- f e
    return $ fRes <*> mRes
