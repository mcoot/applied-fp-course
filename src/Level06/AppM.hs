...
instance Monad (AppM e) where
  ...
  (>>=) (AppM m) f = AppM $ m >>= (either (return . Left) (runAppM . f))
...
instance MonadError e (AppM e) where
  ...
  catchError (AppM m) f = AppM $ m >>= (either (runAppM . f) (return . return))
...
instance Bifunctor AppM where
  ...
  bimap fLeft fRight (AppM m) = AppM $ m >>= return . (B.bimap fLeft fRight)
